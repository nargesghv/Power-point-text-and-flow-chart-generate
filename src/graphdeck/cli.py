...
def synthesize(
    ...
    out_dir: str = typer.Option("out", help="Directory to write outputs"),
    fast: bool = typer.Option(False, "--fast", help="Enable fast mode (no effect here, but kept for symmetry)"),
):
    ...
    ensure_dir(out_dir)
    slug = slugify(topic)
    out = synthesize_bundle(bundle)  # adds "summary"
    # write summary md
...
def blog(
    ...
    content = generate_blog(topic, research=research)  # will use research["summary"] if available
...
def slidechart(
    ...
    research = None
    if research_json and os.path.exists(research_json):
        try: research = json.load(open(research_json, "r", encoding="utf-8"))
    ...
    flowchart_from_title_bullets(
        title=title, bullets=bullets, out_path=out_png, research=research,
        use_llm=(not no_llm), width=width, height=height,
    )
...
def content(
    ...
    if chart_from_slide and outline and isinstance(outline.get("sections"), list):
        ...
        if 0 <= idx < len(outline["sections"]):
            ...
            slide_research = research or research_topic(f"{title} — " + "; ".join((bullets or [])[:6]), max_sources=8, max_images=0)
            flowchart_from_title_bullets(
                title=title, bullets=bullets, out_path=slide_png, research=slide_research,
                use_llm=(not chart_no_llm), width=chart_width, height=chart_height,
            )
            mm_paths.append(slide_png)

    content_files = generate_powerpoint_content(
        topic=topic, outline=outline, research=research, visual_path=(visual_path or None),
        mermaid_paths=mm_paths, out_dir=out_dir, slug=slug,
    )
